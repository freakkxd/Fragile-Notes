# Fragile Notes — v0.1.21

> **Подпись — суть последних коммитов, что в работе и куда идем:**
> - **Последние 5 коммитов:** `f20bd9b` `bump v0.1.21` → `f447673` `remove AllInOne (work only Full)` → `79f10cd` `Full offline bundle GTK` (`collect-all gi`) → `a2f1a86` `fix PyInstaller hang` (`GDK_BACKEND`) → `e402db3` `gcc for C++ helper` / `c18c611` `real exe not bat` + `a197ff2` `bundle libstdc++/Gtk typelib` (фикс `Namespace Gtk` + `libstdc++-6.dll`)
> - **В работе:** `Full Offline 33M` (`PyInstaller` `UCRT64` + `Inno` `33M` `FragileNotes-Setup-v0.1.21.exe` — один файл, `Python+GTK` внутри, без `MSYS2` на юзере) — `Build Windows Full Installer` `success` `f821fc0` 24 сек, сейчас `v0.1.14` `in_progress` с `collect-all gi` + `install_helper.cpp` (`C++` вместо `PowerShell`)
...
Скачай **FragileNotes-Setup-v0.1.21.exe (33M)** из **Releases** → https://github.com/freakkxd/Fragile-Notes/releases/latest
...
- `v0.1.21` — Full Offline 33M самодостаточный, один exe (текущий)
...
A: Старая AllInOne 3.4M требовала `MSYS2` — скачай **Full 33M** из `Releases → v0.1.21` (внутри GTK). Или запусти `AllInOne` с интернетом — он сам скачает `MSYS2` (70M).
