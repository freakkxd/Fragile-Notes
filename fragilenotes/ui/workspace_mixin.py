    ...
    def _on_toggle_sidebar(self, _btn: Gtk.Button | None = None) -> None:
        ...
        if hasattr(self, "top"):
            if vis:
                ...
                # страховка: после анимации Revealer (180ms) ещё раз выставляем
                GLib.timeout_add(220, lambda: self.top.set_position(pos) if self.sidebar_revealer.get_reveal_child() else False)
                try:
                    sc = getattr(self, "side_column", None)
                    if sc is not None:
                        sc.set_size_request(-1, -1)
                        sc.add_css_class("expanded")
                except Exception:
                    pass
            else:
                self.top.set_position(_SIDEBAR_COLLAPSED)
                # форсируем схлопывание после анимации — иначе остаётся пустая полоса 280px как на скрине
                GLib.timeout_add(220, lambda: self.top.set_position(_SIDEBAR_COLLAPSED) if not self.sidebar_revealer.get_reveal_child() else False)
                # side_column должен стать 44px, а не 232px — снимаем expanded и форсим min-width
                try:
                    sc = getattr(self, "side_column", None)
                    if sc is not None:
                        sc.remove_css_class("expanded")
                        # дополнительно сбрасываем size_request чтобы Paned не держал 280px
                        sc.set_size_request(44, -1)
                        GLib.timeout_add(250, lambda: sc.set_size_request(44, -1) if not self.sidebar_revealer.get_reveal_child() else False)
                except Exception:
                    pass
