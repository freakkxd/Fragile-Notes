...
**Сделано:**
- `src-tauri/src/llm.rs` **переписан 544→~700 строк** `schema_version:2` — `Provider {id,kind,name,enabled,endpoint,auth:{method,secret_ref}}` без `api_key`, `Model {id,providerId,name,path,remoteId,capabilities,metadata:{size,sha256,quant,arch,context,chatTemplate,source:gguf|filename|manual}}`, `RuntimeProfile {id,providerId,modelId,executable_source:SystemPath|BundledSidecar,binary_path,port,policy:manual|on-demand|always-on|task-exclusive,settings:LlamaSettings{flashAttention,mlock,numa,noKvOffload,cacheTypeK/V,runtime_args:Vec<String>}}`, `TaskProfile {modelRef,runtimeProfileRef?:null для cloud,privacy:local-only|cloud-allowed,cloudPolicy:Deny|AskOnce|Allow}`, `Capability/ CapabilitySource /Privacy /CloudPolicy`, `RuntimeLimits {max_active:1, allow_concurrent:false}` `порт назначает RuntimeManager 8010/8011...`, `LlmConfig {schema_version, providers,models,runtime_profiles,task_profiles,pipelines,local,runtime_limits}` `RuntimeState {active: [{pid,port,status}]}` отделен (`llm-runtime-state.json`), `migrate_to_v2` с `backup llm.json.bak-v0.5.5` + `atomic_write tmp→fsync→rename`
- `src-tauri/src/llm.rs` **secrets:** `keyring 2.3` `Entry::new("com.fragilich.notes", id)` — `secret_ref keychain://fragile-notes/<id>`, `llm_has/set/delete_provider_credential` + `masked:"••••••••"` для frontend, `llm_get_config` не отдает `apiKey/secretRef`, `save` не пишет plain, при `keyring unavailable` `Err keyring unavailable: [Повторить][Настроить вручную][Отмена]` без fallback в base64, миграция старого `base64 api_key` → `set_keyring` + `secret_ref`
- `src-tauri/src/llm.rs` **typed settings:** `extra_args:String → LlamaAdvancedSettings{flashAttention,mlock,numa,noKvOffload,cacheTypeK/V} + runtime_args:Vec<String>` whitelist, убран shell interpolation
- `src-tauri/src/llm.rs` **Gateway async:** `reqwest::Client async + tokio` `gateway_chat(ChatRequest{provider_id,model_ref,messages,task_profile_id})` + `privacy guard if localOnly && is_cloud → Err PrivacyPolicyViolation` по каждому шагу pipeline, `LlmGateway` единый, `enrich.rs` legacy пойдет через него (след шагом)
- `src-tauri/src/llm.rs` **connection test:** `ConnectionTestResult {ok,provider_id,latency_ms,models,capabilities,error:{code:invalid_key|unauthorized|rate_limited|network|unsupported|server_error,message,retryable}}` — `Local/Ollama GET /health`, `OpenAI GET /v1/models Bearer`, `Gemini GET /v1beta/models?key`, `Claude x-api-key`, `latency`, `retryable`, вместо `len>0`
- `src-tauri/src/llm.rs` **runtime:** `RuntimeManager` `ExecutableSource SystemPath/BundledSidecar` приоритет `user path → bundled → PATH → ошибка с инструкцией`, `RuntimeProfile port` назначается менеджером, `llm_runtime_list/start/stop` + `llm_download_model → Err NotImplemented (curl -L в P1)`, `llm_save_pipeline` блокирует `on_save/scheduled` в P0 (only manual), `pipeline_run` `input_refs` `{{input}}` `{{step_id}}` `outputs:HashMap` + `extract_content` `choices/candidates/content`
- `src-tauri/Cargo.toml` `+ keyring 2.3 + reqwest stream`, `src-tauri/src/main.rs` `+ has/set/delete credential + runtime_list/start/stop` 6 команд, `frontend/src/lib/llm.ts` `zod` tolerant `schema_version, models, runtime_profiles, task_profiles, endpoint/auth/masked` + `llmHas/Set/DeleteProviderCredential`, `LLMSettings.tsx` добавлены `kind/model_ref` для PipelineStep
- Версии → `0.5.6` (`Cargo`, `tauri.conf`, `package.json`, `CMake`, `updater.ts CURRENT`), `tsc` ✅ `vite 1.14s` ✅ (cargo `keyring` требует `libsecret` в CI, локально fallback `keyring unavailable` UI)
- `README.md` подпись `v0.5.6 Foundation P0`, `CHANGELOG.md` этот раздел
