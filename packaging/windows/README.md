# Windows Installer — Fragile Notes v0.1.18
...
> **Актуально:** только **Full Offline 33M** `FragileNotes-Setup-v0.1.18.exe` (внутри `Python 3.11` + `GTK4` + `PyGObject` + все deps). `Portable ZIP` и `AllInOne 3.4M` (исходники + `pip` на машине) — **депрекейтед**, остались в истории `5beb7bb`, больше не собираются.

## Варианты (актуально)

| Файл | Что внутри | Требует | Где брать |
|------|------------|---------|-----------|
| `FragileNotes-Setup-v0.1.18.exe` **33M** | Python + GTK4/libadwaita + PyGObject + PyYAML + cryptography (PyInstaller `UCRT64` + `Inno Setup 6`) | Ничего, один exe | `Releases` → `v0.1.18` `https://github.com/freakkxd/Fragile-Notes/releases/latest` |
...
# → Inno Setup → dist/FragileNotes-Setup-v0.1.18.exe 33M
...
```powershell
signtool sign /fd SHA256 /a dist\FragileNotes-Setup-v0.1.18.exe
```
